...
describe('CostDataDAO', () => {
  ...
  describe('deleteOrphaned', () => {
    it('deletes rows whose account has no assumable_roles and returns change count', async () => {
      vi.mocked(mockStmt.run).mockResolvedValue({ success: true, meta: { changes: 7 } } as unknown as D1Result);
      const dao = new CostDataDAO(mockDb);
      const count = await dao.deleteOrphaned();
      expect(count).toBe(7);
      expect(mockDb.prepare).toHaveBeenCalledWith(expect.stringMatching(/DELETE FROM cost_data.*NOT IN.*assumable_roles/s));
    });
